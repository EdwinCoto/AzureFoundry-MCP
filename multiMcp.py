...
from azure.ai.agents.models import MCPToolResource
...
class AzureAIAgentHandler:
    ...
    async def initialize(self): 
        ...
        try:
            ...
            for tool in self.mcp_tool:
                tool.set_approval_mode("never")  # Disable approval requirement for smoother operation
    ...
    def get_tool_resources(self):
        """Aggregate multiple MCP tool resources into a ToolResources object with required fields."""
        mcp_resources = []
        for tool in self.mcp_tool:
            # Explicitly construct MCPToolResource to ensure server_label is set
        ...
        print(f"Aggregated MCP tool resources: {mcp_resources}")
        return ToolResources(mcp=mcp_resources)
    ...
    def cleanup(self):
        ...
        #    print("Closing project client connection.")
...
async def start():
    ...
    print(f" ************Agent initialization success: {success} ************")
    if success:
        await cl.Message(
            content="✅ Agent initialized successfully! You can now ask questions .",
            author="System"
        ).send()
